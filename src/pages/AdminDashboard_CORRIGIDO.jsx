...
export default function AdminDashboard() {
  ...
  const consolidatedData = {
    matriculated: totalStudents, // Sempre usa o total real de alunos do banco
    absent: 0,
    present: 0,
    visitor: 0,
    bibles: 0,
  ...
  // Filtrar relatórios da data selecionada e somar corretamente
  const reportsForDate = reports.filter(r => r.date === selectedDate);
  
  reportsForDate.forEach(report => {
    // Converter para Number antes de somar para evitar concatenação
    consolidatedData.absent += Number(report.absent) || 0;
    consolidatedData.present += Number(report.present) || 0;
    consolidatedData.visitor += Number(report.visitor) || 0;
    consolidatedData.bibles += Number(report.bibles) || 0;
    consolidatedData.magazines += Number(report.magazines) || 0;
    consolidatedData.offering += Number(report.offering) || 0;

    if (reportsByClass[report.classId]) {
      reportsByClass[report.classId].matriculated = Number(report.matriculated) || 0;
      reportsByClass[report.classId].absent += Number(report.absent) || 0;
      reportsByClass[report.classId].present += Number(report.present) || 0;
      reportsByClass[report.classId].visitor += Number(report.visitor) || 0;
      reportsByClass[report.classId].bibles += Number(report.bibles) || 0;
      reportsByClass[report.classId].magazines += Number(report.magazines) || 0;
      reportsByClass[report.classId].offering += Number(report.offering) || 0;
    }
  });
