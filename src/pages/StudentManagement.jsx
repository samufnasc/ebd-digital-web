...
export default function StudentManagement({ onClose }) {
  ...
  const [formData, setFormData] = useState({ nome: '', classe: '' });
  ...
  const handleSaveStudent = async () => {
    ...
    try {
      let result;
      if (editingId) {
        result = await studentFunctions.updateStudent(editingId, formData.nome, formData.classe);
      } else {
        result = await studentFunctions.addStudent(formData.nome, formData.classe);
      }
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <button
          ...
          className="mb-6 px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition font-semibold"
        >
          + Adicionar Aluno
        </button>
